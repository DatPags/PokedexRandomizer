...
#Region "Images"
    Private Async Function Get_All_Pkmn_Images(pkmnInfo As PkmnInfo) As Task(Of List(Of BitmapImage))
        Dim imgList = New List(Of BitmapImage)
        Dim baseName As String = _imageJson.SelectToken(pkmnInfo.number.ToString("D3")).SelectToken("slug").SelectToken("eng").ToString
        Dim formsToken = _imageJson.SelectToken(pkmnInfo.number.ToString("D3")).SelectToken("gen-8").SelectToken("forms")
        Dim forms = formsToken.Children.ToList
